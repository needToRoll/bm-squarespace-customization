...
function LocatorPlus(configuration) {
    ...
    const buildInfoWindow = function(location) {}
        // Store selection.
    ...
    const markers = locator.locations.map(function(location, index) {
        const icon = {
            url: cdnSpotfinderBaseUrl + "icon/pin-white.png",
        ...
        marker.addListener('click', function() {
            convertLocationToInfoWindow(location).open({
                anchor: marker,
                map,
                shouldFocus: true,
            })
    ...
    locator.renderResultsList = function() {
        ...
        if (locator.searchLocation) {
            sectionNameEl.textContent =
                'Surfspots in der Nähe (' + locations.length + ')';
            locations.sort(function(a, b) {
                return getLocationDistance(a) - getLocationDistance(b);
            });
        } else {
            sectionNameEl.textContent = `Von uns empfolene Surfspots (${locations.length})`;
        }
...
function convertLocationToInfoContentHtmlString(location) {
    ...
    bmIwParentCntainer.innerHTML = contentString
    return bmIwParentCntainer.outerHTML
}
...
function buildPartWithIcon(content, attrName) {
    ...
    attrContainer.appendChild(textElement)
    return attrContainer

}
const difficultyToText = (number) => {
    if (number === 1) {
        return "Einsteiger"
    } else if (number === 2) {
        return "Mittel"
